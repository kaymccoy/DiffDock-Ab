...
# *** had to modify b/c utils was changed to data_train_utils...
#from data.utils import DIPSLoader
from data._utils import DIPSLoader
...
def main(args=None):
    # *** had to modify b/c goodluck.txt was moved
    ...
    for split in ("test", "val", "train"):
        ...
        #*****
        printt("testing n_batches: ", n_batches)
        ...
        for batch_index in tqdm(batch_indexes):
            ...
            #*****
            printt("testing batch_index: ", batch_index)
            ...
            #*****
            printt("args: ", args)
            data = load_data(args, split=split, batch=batch_index, verbose=True)
            ...
            try:
                for i in range(args.generate_n_predictions):
                    ...
                    #*****
                    printt("testing right before sample: ", i)

                    iteration = sample(batch, model, args)

                    #*****
                    printt("testing after sample: ", i)

                    rmsd = evaluate_pose(batch, iteration)["rmsd"]

                    #*****
                    printt("rmsd pose evaluated!")
