...
def load_model(args, model_params, fold, load_best=True, confidence_mode = False, transfer_learning=False):
    """
        Model factory
    ...
    if confidence_mode and args.filtering_model_path is not None:
        ...
        if load_best:
            printt("loaded best model from: ", args.filtering_model_path)
        ...
        if transfer_learning:
            printt("getting optimizer from transfer_model.pth")
            checkpoint = os.path.join(fold_dir, "transfer_model.pth")
        elif load_best:
...
def load_model_for_training(args, model_params, fold, load_best=True, confidence_mode = False, transfer_learning=False):
    """
        Model factory
        :load_best: if True, load best model in terms of performance on val set, else load last model
    """
